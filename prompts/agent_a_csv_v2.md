...
Image_Prompt_Rules (ACTION/PROPS):
  ...
  - 【背景と透過の最適化】 透過作業時の「白抜け（キャラクター内の白色が透明になってしまう現象）」を完全に回避するため、背景は必ず **"gray and white checkerboard background"**（または "transparency grid background"）を指定すること。さらに、切り抜き境界を明瞭にするため、**"clean edges, isolated"** を毎回プロンプトに含めること。
